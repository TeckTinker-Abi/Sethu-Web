...
const NavDropdown = ({ title, items }) => {
  ...
  return (
    ...
    >
      <button 
        className={`flex items-center gap-1.5 relative text-[10px] xl:text-[11px] font-bold uppercase tracking-[0.12em] xl:tracking-[0.15em] transition-colors duration-300 py-4 ${
  ...
  return (
    ...
    >
      <div className="w-full px-4 md:px-10 flex justify-between items-center">
        ...
        {/* Logo Section with Accreditation (Responsive sizes) */}
        <div className="flex items-center gap-2 sm:gap-4 group cursor-pointer flex-shrink-0">
          <div className="relative">
            <img 
              src="images/logo.webp" 
              alt="Sethu Logo" 
              className="h-12 sm:h-16 md:h-20 w-auto transition-all duration-700 group-hover:scale-105" 
            />
          ...
          <div className="border-l border-deep-espresso/10 pl-2 sm:pl-4 flex flex-col justify-center">
            <h1 className="font-serif text-sm sm:text-lg md:text-xl lg:text-2xl tracking-tighter leading-none text-deep-espresso whitespace-nowrap">
        ...
        <div className="hidden min-[1280px]:flex gap-2 xl:gap-4 items-center px-4">
        ...
        <div className="flex items-center gap-3 sm:gap-6 flex-shrink-0">
          <button className="relative group overflow-hidden active:scale-95 transition-all hidden sm:block">
            <span className="absolute -top-1 -right-1 z-30 flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-red-500"></span>
            </span>
            <div className="relative z-10 bg-gold-gradient px-4 py-2 md:px-6 md:py-2.5 shadow-md">
              <span className="relative z-20 text-white text-[10px] md:text-[11px] font-black tracking-widest uppercase">Apply Now</span>
      ...
      <div className={`fixed inset-0 bg-warm-ivory z-[100] transition-all duration-700 min-[1280px]:hidden flex flex-col ${isMobileMenuOpen ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-full pointer-events-none"}`}>
        ...
        <div className="flex flex-col flex-grow overflow-y-auto py-8 px-6 space-y-4">
          ...
          <button className="mt-4 bg-gold-gradient text-white py-4 font-bold tracking-[0.2em] uppercase shadow-xl active:scale-95 transition-transform">
            Apply Now
          </button>
        </div>
      </div>
